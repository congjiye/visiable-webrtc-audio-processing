...
//     = Sony Wave64. format.format = DR_WAVE_FORMAT_PCM;          // <-- Any of the DR_WAVE_FORMAT_* codes.
//     format.channels = 2;
...
#ifndef AUDIO_PROCESSING_DR_WAV_H_
#define AUDIO_PROCESSING_DR_WAV_H_
...
typedef struct {
    ...
    struct {
        drwav_uint64
                iCurrentSample;  // The index of the next sample that will be read by drwav_read_*(). This is used with
        // "totalSampleCount" to ensure we don't read excess samples at the end of the last block.
    } compressed;
...
// Close the loader with drwav_close().
//
// This is the lowest level function for opening a WAV file. You can also use drwav_open_file_write() and
// drwav_open_memory_write() to open the stream from a file or from a block of memory respectively.
//
...
drwav_int16 *drwav_open_and_read_memory_s16(const void *data, size_t dataSize, unsigned int *channels,
                                            unsigned int *sampleRate, drwav_uint64 *totalSampleCount);

float *drwav_open_and_read_memory_f32(const void *data, size_t dataSize, unsigned int *channels,
                                      unsigned int *sampleRate, drwav_uint64 *totalSampleCount);

drwav_int32 *drwav_open_and_read_memory_s32(const void *data, size_t dataSize, unsigned int *channels,
                                            unsigned int *sampleRate, drwav_uint64 *totalSampleCount);
...
#endif
#endif  // AUDIO_PROCESSING_DR_WAV_H_
...
static DRWAV_INLINE short drwav__bytes_to_s16(const unsigned char* data) { return (short)drwav__bytes_to_u16(data); }
...
static DRWAV_INLINE drwav_uint64 drwav__bytes_to_u64(const unsigned char* data) {
    if (drwav__is_little_endian()) {
        ...
        return ((drwav_uint64)data[7] << 0) | ((drwav_uint64)data[6] << 8) | ((drwav_uint64)data[5] << 16) |
               ((drwav_uint64)data[4] << 24) | ((drwav_uint64)data[3] << 32) | ((drwav_uint64)data[2] << 40) |
...
drwav_bool32 drwav_init_memory_write(drwav* pWav, void** ppData, size_t* pDataSize, const drwav_data_format* pFormat) {
    ...
    *pDataSize = 0;
    ...
    if (!drwav_init_write(pWav, pFormat, drwav__on_write_memory, drwav__on_seek_memory_write,
            ...
            if (pWav->container == drwav_container_riff) {
                ...
                if (pWav->onSeek(pWav->pUserData, (int)pWav->dataChunkDataPos + 16, drwav_seek_origin_start)) {
                    drwav_uint64 dataChunkSize =
                        24 + pWav->dataChunkDataSize;  // +24 because W64 includes the size of the GUID and size fields.
        ...
        if (sample > pWav->compressed.iCurrentSample) {
            ...
            while (offset > 0) {
                ...
                if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
                    ...
                    assert(DRWAV_FALSE);  // If this assertion is triggered it means I've implemented a new compressed
        ...
        while (samplesToRead > 0 && pWav->msadpcm.cachedSampleCount > 0 &&
            ...
            pBufferOut[0] =
                (drwav_int16)pWav->msadpcm
                    .cachedSamples[drwav_countof(pWav->msadpcm.cachedSamples) - pWav->msadpcm.cachedSampleCount];
    ...
    while (samplesToRead > 0 && pWav->compressed.iCurrentSample < pWav->totalSampleCount) {
        ...
        if (pWav->ima.cachedSampleCount == 0) {
            if (pWav->ima.bytesRemainingInBlock == 0) {
                ...
                // From what I can tell with stereo streams, it looks like every 4 bytes (8 samples) is for one channel.
                // So it goes 4 bytes for the left channel, 4 bytes for the right channel.
...
static void drwav__ieee_to_f32(float* pOut, const unsigned char* pIn, size_t sampleCount,
                               unsigned short bytesPerSample) {
    if (bytesPerSample == 4) {
        for (unsigned int i = 0; i < sampleCount; ++i) {
            *pOut++ = ((float*)pIn)[i];
...
drwav_uint64 drwav_read_f32__msadpcm(drwav* pWav, drwav_uint64 samplesToRead, float* pBufferOut) {
    // We're just going to borrow the implementation from the drwav_read_s16() since ADPCM is a little bit more
...
#ifdef DR_WAV_LIBSNDFILE_COMPAT
    // It appears libsndfile uses slightly different logic for the u8 -> f32 conversion to dr_wav, which in my opinion
    // is incorrect. It appears libsndfile performs the conversion something like "f32 = (u8 / 256) * 2 - 1", however I
    // think it should be "f32 = (u8 / 255) * 2 - 1" (note the divisor of 256 vs 255). I use libsndfile as a benchmark
    // for testing, so I'm therefore leaving this block here just for my automated correctness testing. This is disabled
    // by default.
    for (size_t i = 0; i < sampleCount; ++i) {
...
//
// v0.5 - 2016-09-29
//   - API CHANGE. Swap the order of "channels" and "sampleRate" parameters in drwav_open_and_read*(). Rationale for
//   this is to
